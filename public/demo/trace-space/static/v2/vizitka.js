// Vizitka — meni se razpre cez celo stran in postane platno.
//
// Modul je namenoma locen od app.js in se nalozi sele ob prvem odprtju:
// nosi tri animacije (duhec, epruveta, ASCII morph), ki jih nadzorna plosca
// pri delu ne potrebuje. Nic od tega ne gre v omrezje - slike so lokalne,
// pisave sistemske.
...
function duhecMarkup(pot) {
  const face = (n, on) =>
    `<img class="duh-face${on ? " on" : ""}" data-face="${n}" src="${pot}/duhec-${n}.png" alt="">`;
  return (
    '<span class="duh">' +
    `<img class="duh-body" src="${pot}/duhec-body.png" alt="">` +
...
function duhecPoleti(pot, izhod) {
  const VEL = 68;
  const el = document.createElement("div");
  el.className = "duh-let";
  el.innerHTML = duhecMarkup(pot);
...
export function mountVizitka(host, opts = {}) {
  const pot = opts.imgPath || "static/v2/img";
  ...
  function robMenija() {
    ...
    const r = s ? s.getBoundingClientRect() : { top: 0, right: 224, bottom: innerHeight, left: 0 };
    return `inset(${Math.max(0, r.top)}px ${Math.max(0, innerWidth - r.right)}px ` +
      `${Math.max(0, innerHeight - r.bottom)}px ${Math.max(0, r.left)}px round 0px 22px 22px 0px)`;
  }

  function odpri() {
    if (odprta) return;
    const meni = document.querySelector(".sidebar");
    const sheet = document.createElement("div");
    sheet.className = "viz-sheet";
    sheet.innerHTML = vizitkaMarkup(opro);
    document.body.appendChild(sheet);
    document.documentElement.classList.add("viz-open");
    if (meni) meni.classList.add("viz-prazen");
    ...
    let duh = null;
    if (!mir) {
      duh = duhecPoleti(pot, btn.querySelector(".duh").getBoundingClientRect());
    ...
    if (!mir) {
      sheet.animate([{ clipPath: robMenija() }, { clipPath: "inset(0px 0px 0px 0px round 0px)" }],
    ...
    odprta = { sheet, meni, stop, naTipko, duh };
    btn.classList.add("on");
  }

  function zapri() {
    if (!odprta) return;
    const { sheet, meni, stop, naTipko, duh } = odprta;
    odprta = null;
    ...
    const konec = () => {
      ...
      if (meni) meni.classList.remove("viz-prazen");
    };
    if (reduced()) return konec();
    sheet.querySelector(".viz-card").animate(
      [{ opacity: 1, filter: "blur(0px)" }, { opacity: 0, filter: "blur(10px)" }],
      { duration: 260, easing: "ease-in", fill: "forwards" });
    const a = sheet.animate([{ clipPath: "inset(0px 0px 0px 0px round 0px)" }, { clipPath: robMenija() }],
